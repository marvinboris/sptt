...
export default async function handler(
  ...
  if (req.method === "GET") {
    if (slug[0] === "info") return manage.info();
      ...
      const role = await Role.findById(slug[0]).populate<{
        feature: FeatureInterface;
      }>("features.feature");
      if (!role)
        return res.json({
          message: message(cms.backend.messages.roles.not_found, "danger"),
        });
      ...
      fields: {
        features: (fields) => {
          const allFields = Object.keys(fields)
            .filter((field) => field.includes("features"))
            .map((field) =>
              field
                .split("[")
                .filter((_c, i) => i > 0)
          ...
          const features = allFields
            .filter(([_id, type]) => type === "id")
            .map(([featureId]) => {
              const access = allFields
                .filter(
                  ([permissionId, type]) =>
                    featureId === permissionId && type === "permissions"
                )
                .map(([, , access]) => access);
