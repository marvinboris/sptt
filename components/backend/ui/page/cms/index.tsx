...
type AddParams = {
  regex: string;
  action: (item: string | StringObject) => ReactNode;
}[];

const recursiveDeepness = (
  paramItem: StringObject,
  paramName: string,
  paramId: string,
  paramValue: string | StringObject,
  paramDeepness: string[],
  paramPrepends: AddParams = [],
  paramAppends: AddParams = []
): ReactNode[] =>
  Object.keys(paramItem).map((item) => {
    ...
    const findPrepend = paramPrepends.find((el) =>
      new RegExp(el.regex.replace(/\[/g, "\\[").replace(/\]/g, "\\]")).test(
        mainName
      )
    );
    ...
    return typeof mainItem === "string" ? (
      <Fragment key={Math.random() + mainName}>
        {prepend}
        <TextArea
          key={`${Math.random() + mainName}-input`}
          name={mainName}
          id={mainId}
          label={
            <span title={htmlEntities(mainItem) as string}>
              {htmlEntities(mainItem) as string}
            </span>
          }
          placeholder={htmlEntities(mainItem) as string}
          defaultValue={mainValue as string}
        />
        {append}
      </Fragment>
    ) : (
      recursiveDeepness(
        mainItem as StringObject,
        mainName,
        mainId,
        mainValue as string | StringObject,
        mainDeepness,
        paramPrepends,
        paramAppends
      )
...
export const WithPages = ({ cmsExample, cmsValue, part }: WithPagesProps) => {
  const [activeSection, setActiveSection] = useState(
    Object.keys(cmsExample).sort()[0]
  );

  const sectionsOptions = Object.keys(cmsValue)
    .map((key) => {
      let name = capitalize(key.split("_").join(" "));
      if (Object.keys(cmsExample.pages).find((k) => k === key))
        name = `Page - ${name}`;
  ...
  const nonPagesContent = nonPagesKeys.map((item) => (
    <div
      key={`${Math.random()}${prefix}[${item}]`}
      className={`grid gap-y-2 gap-x-4 md:col-span-2 md:grid-cols-2 xl:col-span-3 xl:grid-cols-3 pt-4${
        item === activeSection ? "" : " hidden"
      }`}
    >
  ...
  const pagesContent = pagesKeys.map((item) => (
    <div
      key={`${Math.random()}${prefix}[pages][${item}]`}
      className={`grid gap-y-2 gap-x-4 md:col-span-2 md:grid-cols-2 xl:col-span-3 xl:grid-cols-3 pt-4${
        item === activeSection ? "" : " hidden"
      }`}
    >
  ...
  return (
    <>
      ...
      <div className="py-4 md:col-span-2 xl:col-span-3">
        <hr />
      </div>
